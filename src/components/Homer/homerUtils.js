...
export function getSummaryStats(calculatedColumns, modelInputs) {
  ...
  const newApplianceYearlyKwh = sumGreaterThanZero(
    calculatedColumns,
    'newApplianceLoad'
  )
  ...
  const newApplianceUnmetLoadCost =
    additionalUnmetLoadSum * modelInputs['unmetLoadCostPerKwh']

  const newApplianceNetRevenue =
    newApplianceElectricityRevenue -
    newApplianceElectricityCost -
    newApplianceUnmetLoadCost

  // Calculate production of new appliance based on
  const yearlyProductionUnits =
    newApplianceYearlyKwh * modelInputs['productionUnitsPerKwh']
  const yearlyProductionUnitsRevenue =
    yearlyProductionUnits * modelInputs['revenuePerProductionUnits']
  const netApplianceOwnerRevenue =
    yearlyProductionUnitsRevenue - newApplianceElectricityRevenue
